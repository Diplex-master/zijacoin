...
#ifndef ZIJA_ZDLXWALLET_H
#define ZIJA_ZDLXWALLET_H
...
public:
    CzDLXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDLX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDLX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ZIJA_ZDLXWALLET_H
