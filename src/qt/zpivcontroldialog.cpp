...
std::set<std::string> ZDiplexCoinControlDialog::setSelectedMints;
std::set<CMintMeta> ZDiplexCoinControlDialog::setMints;

ZDiplexCoinControlDialog::ZDiplexCoinControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDiplexCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDiplexCoinControlDialog::~ZDiplexCoinControlDialog()
{
    delete ui;
}

void ZDiplexCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDiplexCoinControlDialog::updateList()
{
...
void ZDiplexCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDiplexCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDiplexCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDiplexCoinControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDiplexCoinControlDialog::GetSelectedMints()
{
...
void ZDiplexCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
